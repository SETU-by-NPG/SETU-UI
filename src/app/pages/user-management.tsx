...
import { Plus, Upload, Download, X, MoreVertical, Search, Filter, UserPlus, Mail, ShieldCheck, ShieldOff } from "lucide-react";
import { DataTable } from "../components/data-table";
import { StatusBadge } from "../components/status-badge";
import { systemUsers, type SystemUser } from "../data/mock-data";

const roleFilterOptions = ["All", "admin", "teacher", "student", "parent", "staff"] as const;
...
export default function UserManagementPage() {
  ...
  const columns = [
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
            <span style={{ fontSize: "0.75rem", fontWeight: 600 }}>
              {u.name.split(" ").filter(n => !["Mr.", "Ms.", "Dr."].includes(n)).map(n => n[0]).join("").slice(0, 2)}
            </span>
          </div>
    ...
    },
    {
      ...
      label: "Role",
      render: (u: SystemUser) => (
        <span className="px-2 py-0.5 rounded-full bg-muted capitalize" style={{ fontSize: "0.75rem", fontWeight: 500 }}>
          {u.role}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (u: SystemUser) => (
        <div className="relative">
          <button
            onClick={(e) => { e.stopPropagation(); setShowActionMenu(showActionMenu === u.id ? null : u.id); }}
    ...
    <div onClick={() => setShowActionMenu(null)}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <div className="bg-card border border-border rounded-xl p-4 text-center">
          <p className="text-muted-foreground" style={{ fontSize: "0.75rem" }}>Total Users</p>
          <p style={{ fontSize: "1.5rem", fontWeight: 600 }}>{stats.total}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4 text-center">
          <p className="text-muted-foreground" style={{ fontSize: "0.75rem" }}>Active</p>
          <p className="text-green-600" style={{ fontSize: "1.5rem", fontWeight: 600 }}>{stats.active}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4 text-center">
          <p className="text-muted-foreground" style={{ fontSize: "0.75rem" }}>Suspended</p>
          <p className="text-red-600" style={{ fontSize: "1.5rem", fontWeight: 600 }}>{stats.suspended}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4 text-center">
          <p className="text-muted-foreground" style={{ fontSize: "0.75rem" }}>Pending</p>
          <p className="text-amber-600" style={{ fontSize: "1.5rem", fontWeight: 600 }}>{stats.pending}</p>
        </div>
      </div>
        ...
        {roleFilterOptions.map((r) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-md capitalize whitespace-nowrap transition-colors ${
              roleFilter === r ? "bg-card shadow-sm" : "text-muted-foreground hover:text-foreground"
            }`}
      ...
      <DataTable
        data={filteredUsers}
        columns={columns}
        searchKey={(u) => `${u.name} ${u.email} ${u.role} ${u.department || ""}`}
          ...
          <div className="bg-card rounded-xl border border-border shadow-xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                ...
                <label htmlFor="sendInvite" style={{ fontSize: "0.875rem" }}>Send invitation email with temporary password</label>
              </div>
              <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                <input type="checkbox" id="require2fa" className="rounded" />
                <label htmlFor="require2fa" style={{ fontSize: "0.875rem" }}>Require two-factor authentication</label>
              </div>
            </div>
            <div className="flex justify-end gap-3 p-5 border-t border-border">
              ...
              <button onClick={() => setShowAddModal(false)} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:opacity-90" style={{ fontSize: "0.875rem" }}>Create User</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedUser && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setSelectedUser(null)}>
          <div className="bg-card rounded-xl border border-border shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-border">
              <h2>User Details</h2>
              <button onClick={() => setSelectedUser(null)} className="p-1 rounded-md hover:bg-muted">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5">
              <div className="flex items-center gap-4 mb-5">
                <div className="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center">
                  <span style={{ fontSize: "1.25rem", fontWeight: 600 }}>
                    {selectedUser.name.split(" ").filter(n => !["Mr.", "Ms.", "Dr."].includes(n)).map(n => n[0]).join("").slice(0, 2)}
                  </span>
                </div>
                <div>
                  <p style={{ fontWeight: 600 }}>{selectedUser.name}</p>
                  <p className="text-muted-foreground" style={{ fontSize: "0.875rem" }}>{selectedUser.email}</p>
                </div>
              </div>
              <div className="space-y-3">
                {[
                  ...
                  <div key={field.label} className="flex justify-between py-2 border-b border-border last:border-b-0">
                    <span className="text-muted-foreground" style={{ fontSize: "0.875rem" }}>{field.label}</span>
                    <span className="capitalize" style={{ fontSize: "0.875rem", fontWeight: 500 }}>{field.value}</span>
                  </div>
                ))}
              </div>
            </div>
