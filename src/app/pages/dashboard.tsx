...
const widgetsByRole: Partial<Record<Role, WidgetDef[]>> = {
  // Administrative roles
  master_admin: adminWidgets,
  it_admin: adminWidgets,
  it_technician: adminWidgets,
  principal: adminWidgets,
  finance_manager: adminWidgets,
  hr_manager: adminWidgets,
  admissions_officer: adminWidgets,
  data_manager: adminWidgets,
  facilities_manager: adminWidgets,
  // Academic leadership
  slt_member: adminWidgets,
  head_of_department: teacherWidgets,
  head_of_year: teacherWidgets,
  examinations_officer: teacherWidgets,
  // Safeguarding & welfare
  safeguarding_lead: teacherWidgets,
  senco: teacherWidgets,
  attendance_officer: teacherWidgets,
  careers_advisor: teacherWidgets,
  // Teaching
  teacher: teacherWidgets,
  cover_supervisor: teacherWidgets,
  teaching_assistant: teacherWidgets,
  // Technical specialists & support
  librarian: librarianWidgets,
  science_technician: librarianWidgets,
  subject_technician: librarianWidgets,
  support_staff: librarianWidgets,
  // Students
  student: studentWidgets,
  student_leadership: studentWidgets,
  // Parent
...
function useWidgetVisibility(role: Role) {
  ...
  const defs = widgetsByRole[role] ?? adminWidgets;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="w-full mt-4 px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:opacity-90"
...
function AdminDashboard() {
  const { isVisible, visibility, toggle, defs } = useWidgetVisibility("master_admin");
  const [showCustomize, setShowCustomize] = useState(false);
  const recentAlerts = auditLogs
    .filter((l) => l.severity === "warning" || l.severity === "critical")
    .slice(0, 4);
  const allOperational = serviceStatuses.every(
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground" style={{ fontSize: "0.875rem" }}>
            {subscription.orgName} &middot;{" "}
            <span className="font-mono">{subscription.tenantId}</span>
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-2 mr-2">
            <div
              className={`w-2 h-2 rounded-full ${allOperational ? "bg-green-500" : "bg-amber-500"}`}
            />
            <span
      ...
      {isVisible("subscription") && (
        <div className="bg-card border border-border rounded-xl p-5 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                  <span
                    ...
                    style={{ fontSize: "0.8125rem" }}
                  >
                    <MapPin className="w-3.5 h-3.5" /> {subscription.region}
                  </span>
                  <span
                    className="text-muted-foreground"
                    style={{ fontSize: "0.8125rem" }}
            ...
            <a
              href="/settings"
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border hover:bg-muted transition-colors shrink-0"
              style={{ fontSize: "0.8125rem" }}
            >
      ...
      {isVisible("quotas") && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {(Array.isArray(resourceQuotas) ? resourceQuotas : []).map((q) => {
              const percent = Math.round((q.used / q.limit) * 100);
              const isHigh = percent > 80,
                isMedium = percent > 60 && percent <= 80;
              return (
                <div
                  key={q.resource}
                ...
                >
                  ...
                  <div className="w-full h-2 bg-muted rounded-full overflow-hidden mb-2">
                    <div
                      className={`h-full rounded-full ${isHigh ? "bg-red-500" : isMedium ? "bg-amber-500" : "bg-primary"}`}
                      style={{ width: `${percent}%` }}
                    />
                  </div>
                  <p
      ...
      {isVisible("stats") && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard
            ...
            value={systemMetrics.activeSessionsNow}
          ...
          <StatCard
            ...
            value={systemMetrics.failedLogins24h}
            icon={<AlertTriangle className="w-5 h-5 text-muted-foreground" />}
            change="1 IP auto-blocked"
            changeType="negative"
          />
          <StatCard
            label="Open Tickets"
            value={openTickets.length}
            icon={<TicketCheck className="w-5 h-5 text-muted-foreground" />}
            change={`${tickets.filter((t) => t.priority === "critical" && t.status !== "resolved" && t.status !== "closed").length} critical`}
            changeType="negative"
          />
          <StatCard
        ...
        {isVisible("services") && (
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span
                className={`px-2 py-0.5 rounded-full text-white ${allOperational ? "bg-green-600" : "bg-amber-600"}`}
                style={{ fontSize: "0.65rem", fontWeight: 500 }}
              >
                {allOperational ? "ALL OK" : "ISSUES"}
              </span>
            </div>
              ...
              {serviceStatuses.map((svc) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span
                      className={`w-2 h-2 rounded-full ${statusColor[svc.status]}`}
                    />
                    <span style={{ fontSize: "0.8125rem" }}>{svc.name}</span>
                  </div>
                  <span
                    className={`px-1.5 py-0.5 rounded ${svc.status === "operational" ? "bg-green-50 text-green-700" : svc.status === "degraded" ? "bg-amber-50 text-amber-700" : svc.status === "maintenance" ? "bg-blue-50 text-blue-700" : "bg-red-50 text-red-700"}`}
            ...
            <p
              className="text-muted-foreground mt-3 pt-3 border-t border-border"
        ...
        {isVisible("users-role") && (
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <p
              ...
              style={{ fontSize: "0.75rem" }}
            >
              {systemMetrics.totalUsers} / {subscription.maxUsers} seats
            </p>
          </div>
        )}
        {isVisible("tickets") && (
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="space-y-2">
              {openTickets.slice(0, 4).map((t) => (
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${t.priority === "critical" ? "bg-red-500" : t.priority === "high" ? "bg-orange-500" : "bg-amber-500"}`}
                  />
                  <div className="min-w-0 flex-1">
                    ...
                    <p
                      ...
                      style={{ fontSize: "0.75rem" }}
                    >
                      {t.ticketNo} &middot; {t.createdBy} &middot; {t.createdAt}
                    </p>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-card border border-border rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {recentAlerts.map((log) => (
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${log.severity === "critical" ? "bg-red-500" : "bg-amber-500"}`}
                />
...
function TeacherDashboard() {
  const { isVisible, visibility, toggle, defs } =
    useWidgetVisibility("teacher");
  const [showCustomize, setShowCustomize] = useState(false);
  const myAssignments = assignments.filter((a) =>
    ["10-A", "11-A", "12-A"].includes(a.class),
  );
  const myTickets = tickets.filter(
    (t) => t.createdBy.includes("Williams") || t.createdBy.includes("John"),
  );
  const myBookings = equipmentBookings.filter(
    (b) => b.bookedBy.includes("Williams") || b.bookedBy.includes("John"),
  );
      ...
      {isVisible("stats") && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            ...
            value={myAssignments.filter((a) => a.status !== "graded").length}
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            {myTickets.length > 0 ? (
              myTickets.slice(0, 3).map((t) => (
                <div
                  key={t.id}
                  className="flex items-center justify-between p-3 rounded-lg border border-border mb-2"
                >
                  <div className="min-w-0">
                    <p style={{ fontWeight: 500, fontSize: "0.875rem" }}>
                      {t.title}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p
                      ...
                      style={{ fontSize: "0.75rem" }}
                    >
                      {slot.teacher} &middot; Room {slot.room}
                    </p>
                  </div>
                </div>
              ))}
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p
                        ...
                        style={{ fontSize: "0.75rem" }}
                      >
                        {a.subject} &middot; Due {a.dueDate}
                      </p>
                    </div>
                    <StatusBadge status={a.status} />
                  </div>
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1>Dashboard</h1>
          <p className="text-muted-foreground" style={{ fontSize: "0.875rem" }}>
            Hello, Robert. Here's how Alice is doing.
          </p>
        </div>
        <button
          onClick={() => setShowCustomize(true)}
        ...
        <div className="bg-card border border-border rounded-xl p-5 mb-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
              <span style={{ fontWeight: 600, fontSize: "1.125rem" }}>AJ</span>
            </div>
            <div>
              <p style={{ fontWeight: 600 }}>Alice Johnson</p>
              <p
                className="text-muted-foreground"
                style={{ fontSize: "0.875rem" }}
              >
                Grade 10 &middot; Section A &middot; Roll #1001
              </p>
            </div>
          </div>
        </div>
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1>Dashboard</h1>
          <p className="text-muted-foreground" style={{ fontSize: "0.875rem" }}>
            Welcome, Ms. Nair. Manage the library catalog.
          </p>
        </div>
        <button
          onClick={() => setShowCustomize(true)}
...
export default function DashboardPage() {
  ...
  const librarianRoles: Role[] = [
    "librarian",
    "science_technician",
    "subject_technician",
    "support_staff",
  ];
  const studentRoles: Role[] = ["student", "student_leadership"];

  if (adminRoles.includes(role)) return <AdminDashboard />;
  if (teacherRoles.includes(role)) return <TeacherDashboard />;
  if (librarianRoles.includes(role)) return <LibrarianDashboard />;
  if (studentRoles.includes(role)) return <StudentDashboard />;
