...
import type { Role, Permission, AccessConfig, Teacher, AuditLog } from '../types';
import { DEFAULT_PERMISSIONS, getConfigurablePermissionsForRole } from '../data/permissions';
...
const {
  ...
  const [auditLog, setAuditLog] = useState<PermissionChangeEntry[]>(MOCK_AUDIT_LOG);
  const [activeTab, setActiveTab] = useState('overview');
  const [selectedRole, setSelectedRole] = useState<Role>('teacher');
  const [showChangeConfirmation, setShowChangeConfirmation] = useState(false);
  const [lastChange, setLastChange] = useState<PermissionChangeEntry | null>(null);

  // Teacher-specific configurations
  const [teacherConfigs, setTeacherConfigs] = useState<TeacherPermissionConfig[]>(
  ...
  const [reportVisibility, setReportVisibility] = useState<ReportVisibilityConfig>({
    admin: true,
    teacher: true,
    student: false,
    parent: false,
  ...
  const addAuditEntry = useCallback((entry: Omit<PermissionChangeEntry, 'id' | 'timestamp'>) => {
    ...
    console.log('%c[Access Management] Permission Change Logged:', 'color: #4CAF50; font-weight: bold;', {
      action: entry.action,
  ...
  const handleRolePermissionToggle = useCallback((permission: Permission, role: Role, granted: boolean) => {
    const existingConfig = accessConfigs.find(
      (c: AccessConfig) => c.permissionKey === permission.key && c.role === role
    );
  ...
  const handleTeacherPermissionToggle = useCallback((teacherId: string, permissionKey: string, granted: boolean) => {
    ...
    console.log('%c[Access Management] Teacher Permission Updated:', 'color: #2196F3; font-weight: bold;', {
      teacher: teacher?.name,
      permission: permissionKey,
      granted,
      effect: granted ? 'Teacher can now use this feature' : 'Teacher can no longer use this feature',
    });
  }, [addAuditEntry]);

  // Handle librarian config toggle
  const handleLibrarianConfigToggle = useCallback((key: keyof LibrarianConfig, value: boolean) => {
    ...
    console.log('%c[Access Management] Librarian Config Updated:', 'color: #9C27B0; font-weight: bold;', {
      permission: permissionNames[key],
      enabled: value,
      affectedModules: ['Library Management', 'Book Catalog', 'Student Access Control'],
    });
  }, [addAuditEntry]);

  // Handle report visibility toggle
  const handleReportVisibilityToggle = useCallback((role: keyof ReportVisibilityConfig, value: boolean) => {
    ...
    console.log('%c[Access Management] Report Visibility Updated:', 'color: #FF9800; font-weight: bold;', {
      role,
      canViewReports: value,
      affectedReports: ['Student Academic Reports', 'Attendance Reports', 'Behavior Reports'],
    });
  }, [addAuditEntry]);

  // Get effective permission status
  const getEffectivePermission = useCallback((permissionKey: string, role: Role): boolean => {
    const config = accessConfigs.find((c: AccessConfig) => c.permissionKey === permissionKey && c.role === role);
    if (config) {
    ...
    return permission?.defaultAccess[role] === 'allow';
  }, [accessConfigs]);
  ...
  const PermissionToggle = ({ 
    ...
    const affectedModules = useMemo(() => {
      ...
      if (permission.key.includes('assignment')) modules.push('Assignments', 'Gradebook');
      if (permission.key.includes('gradebook')) modules.push('Gradebook', 'Reports');
      if (permission.key.includes('timetable')) modules.push('Timetable', 'Scheduling');
      if (permission.key.includes('library')) modules.push('Library', 'Book Management');
    ...
    return (
      <div className="flex items-start justify-between p-4 border rounded-lg mb-3 bg-card hover:bg-accent/5 transition-colors">
        ...
        <Switch
          checked={isGranted}
          onCheckedChange={(checked) => handleRolePermissionToggle(permission, role, checked)}
          disabled={!isAdmin}
        />
      </div>
    );
      ...
      {showChangeConfirmation && lastChange && (
        <Alert className="mb-6 bg-green-50 border-green-200">
          ...
          <AlertDescription className="text-green-700">
            {lastChange.action === 'grant' ? 'Granted' : 'Revoked'} {lastChange.permissionName} 
            {lastChange.targetUserName ? ` for ${lastChange.targetUserName}` : ` for ${lastChange.role}s`}
          </AlertDescription>
        </Alert>
      )}
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-3xl font-bold">{configurablePermissions.length}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-3xl font-bold">
                  {accessConfigs.filter((c: AccessConfig) => c.access === 'allow').length}
                </div>
                <p className="text-sm text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <Tabs defaultValue="teacher">
                ...
                {(['teacher', 'student', 'parent', 'librarian'] as Role[]).map(role => (
                      ...
                      <div className="pr-4">
                        {configurablePermissions.length === 0 ? (
                          <Alert>
                            ...
                            <AlertTitle>No Configurable Permissions</AlertTitle>
                            <AlertDescription>
                              There are no configurable permissions available for {role}s.
                            </AlertDescription>
                          </Alert>
                        ) : (
        ...
        <TabsContent value="teachers" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure individual permissions for each teacher. These settings override role-based defaults.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Accordion type="multiple" className="space-y-4">
                {teacherConfigs.map(teacher => (
                  <AccordionItem key={teacher.teacherId} value={teacher.teacherId}>
                    <AccordionTrigger className="hover:no-underline">
                      <div className="flex items-center justify-between w-full pr-4">
                        ...
                        <Badge variant="outline">
                          {Object.entries(teacher.permissions).filter(([_, v]) => v).length} / {Object.keys(teacher.permissions).length} permissions
                        </Badge>
                      </div>
                    </AccordionTrigger>
                    <AccordionContent>
                      <div className="pt-4 space-y-6">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {[
                              { key: 'academic.assignment.create', label: 'Create Assignments' },
                              { key: 'academic.assignment.edit', label: 'Edit Assignments' },
                              { key: 'academic.assignment.grade', label: 'Grade Assignments' },
                              { key: 'academic.assignment.delete', label: 'Delete Assignments' },
                            ].map(({ key, label }) => (
                              <div key={key} className="flex items-center justify-between p-3 border rounded-lg">
                                <span className="text-sm">{label}</span>
                                <Switch
                                  checked={teacher.permissions[key as keyof typeof teacher.permissions]}
                                  onCheckedChange={(checked) => 
                                    handleTeacherPermissionToggle(teacher.teacherId, key, checked)
                                  }
                                />
                              </div>
                            ))}
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            {[
                              { key: 'academic.gradebook.view', label: 'View Gradebook' },
                              { key: 'academic.gradebook.edit', label: 'Edit Grades' },
                              { key: 'academic.gradebook.export', label: 'Export Grades' },
                            ].map(({ key, label }) => (
                              <div key={key} className="flex items-center justify-between p-3 border rounded-lg">
                                <span className="text-sm">{label}</span>
                                <Switch
                        ...
                        <div>
                          ...
                          <div className="flex items-center justify-between p-3 border rounded-lg">
                            <span className="text-sm">Edit Student Notes in Timetable</span>
                            <Switch
                              checked={teacher.permissions['academic.timetable.notes.edit']}
                              onCheckedChange={(checked) => 
                                handleTeacherPermissionToggle(teacher.teacherId, 'academic.timetable.notes.edit', checked)
                              }
                            />
                          </div>
                        </div>
        ...
        <TabsContent value="librarians" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure permissions specific to librarian role for library management.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      handleLibrarianConfigToggle('canManageLibraryAccess', checked)
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      handleLibrarianConfigToggle('canManageBookCatalog', checked)
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      handleLibrarianConfigToggle('canViewBorrowingHistory', checked)
                    }
                  />
                </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  When "Manage Library Access" is enabled, librarians can control which students 
                  are allowed to borrow books. This affects the Library module's access control features.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
        ...
        <TabsContent value="reports" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure which roles can view student reports and academic records.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {([
                  { key: 'admin', label: 'Administrators', description: 'Full access to all reports' },
                  { key: 'teacher', label: 'Teachers', description: 'Access to their students\' reports' },
                  { key: 'student', label: 'Students', description: 'Access to their own reports only' },
                  { key: 'parent', label: 'Parents', description: 'Access to their children\'s reports' },
                  { key: 'librarian', label: 'Librarians', description: 'Access to library-related reports' },
              ...
              <div className="mt-6 p-4 bg-muted rounded-lg">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Academic Performance', 'Attendance Summary', 'Behavior Records', 'Progress Reports', 'Transcripts'].map(report => (
                    <Badge key={report} variant="secondary">{report}</Badge>
                  ))}
                </div>
              </div>
            </CardContent>
                ...
                <TableBody>
                  {auditLog.map(entry => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell>
                        <Badge 
                          variant={entry.action === 'grant' ? 'default' : entry.action === 'revoke' ? 'destructive' : 'secondary'}
                      ...
                      <TableCell>
                        {entry.oldValue ? (
                          <Badge variant="outline" className={entry.oldValue === 'allow' ? 'text-green-600' : 'text-red-600'}>
                            {entry.oldValue}
                          </Badge>
                        ) : (
                      ...
                      <TableCell>
                        <Badge variant="outline" className={entry.newValue === 'allow' ? 'text-green-600' : 'text-red-600'}>
                          {entry.newValue}
                        </Badge>
                      </TableCell>
                    </TableRow>
