...
const modules = [
  {
    ...
    description: "Student profiles, enrollment, records",
    enabled: true,
    core: true,
  },
  {
    ...
    description: "Teacher profiles and class assignments",
    enabled: true,
    core: true,
  },
  {
    id: "m3",
    name: "Attendance",
    description: "Daily attendance marking and tracking",
    enabled: true,
    core: true,
  },
  {
    id: "m4",
    name: "Assignments",
    description: "Assignment creation, submission, grading",
    enabled: true,
    core: false,
  },
  {
    id: "m5",
    name: "Gradebook",
    description: "Grade management and report cards",
    enabled: true,
    core: false,
  },
  {
    id: "m6",
    name: "Timetable",
    description: "Class scheduling and room management",
    enabled: true,
    core: false,
  },
  {
    ...
    description: "Academic reports and data analytics",
    enabled: true,
    core: false,
  },
  {
    id: "m8",
    name: "Announcements",
    description: "School-wide notifications and notices",
    enabled: true,
    core: false,
  },
  {
  ...
  },
  {
    ...
    description: "Book catalog, issuing, and returns",
    enabled: false,
    core: false,
  },
];
...
export default function SettingsPage() {
  const { currency, libraryFinesEnabled, setCurrency, setLibraryFinesEnabled } =
    useAppSettings();
  const { role } = useOutletContext<{ role: string }>();
  const [saved, setSaved] = useState(false);
  const [activeTab, setActiveTab] = useState(
    role === "admin" ? "overview" : "school",
  );
      ...
      <div className="flex h-[calc(100vh-8rem)] -m-4 lg:-m-6">
        ...
        <div className="w-56 bg-card border-r border-border shrink-0 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-border">
            ...
            >
              {(subscription as { orgName?: string }).orgName || "SETU Academy"}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-8">
          ...
          {activeTab === "overview" && (
            <div>
              <div className="mb-6">
                ...
                >
                  Your SETU instance at a glance.
                </p>
              </div>
              ...
              <div className="bg-card border border-border rounded-xl p-6 mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    {
                      label: "Plan",
                      value: (subscription as { plan?: string }).plan || "-",
                    },
                    {
                      label: "Status",
                      value:
                    ...
                    },
                    {
                      label: "Renewal",
                      value:
                        (subscription as { renewalDate?: string })
                          .renewalDate || "-",
                    },
                    {
                      label: "Features",
                  ...
                  {[
                    {
                      label: "Max Users",
                      value:
                        typeof (subscription as { maxUsers?: number })
                          .maxUsers === "number"
                          ? (
                              subscription as { maxUsers?: number }
                    ...
                    },
                    {
                      label: "Storage",
                      value:
                        (subscription as { maxStorage?: string }).maxStorage ||
                        "100 GB",
                    },
                    {
                      label: "Daily API Calls",
                      value:
                        typeof (subscription as { maxApiCalls?: number })
                          .maxApiCalls === "number"
                          ? (
                              subscription as { maxApiCalls?: number }
                            ).maxApiCalls?.toLocaleString()
                          : "100,000",
                    },
                  ].map((item) => (
                    <div
                  ...
                  {serviceStatuses.map((svc) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span
                          className={`w-2.5 h-2.5 rounded-full ${svc.status === "operational" ? "bg-green-500" : svc.status === "degraded" ? "bg-amber-500" : svc.status === "maintenance" ? "bg-blue-500" : "bg-red-500"}`}
                        />
                        <span style={{ fontSize: "0.875rem" }}>{svc.name}</span>
                      </div>
                      <div className="flex items-center gap-4">
                        {(svc as { latency?: string }).latency &&
                          (svc as { latency?: string }).latency !== "â€”" && (
                            <span
                              className="text-muted-foreground"
                              style={{ fontSize: "0.75rem" }}
                            >
            ...
            <div>
              <div className="mb-6">
                <h1 style={{ fontSize: "1.25rem" }}>
                  {role === "admin" ? "Organization Profile" : "School Profile"}
                </h1>
                <p
                  className="text-muted-foreground mt-1"
                  style={{ fontSize: "0.875rem" }}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card border border-border rounded-xl p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      defaultValue="123 Education Lane, Bangalore, India"
                      className="w-full mt-1 px-3 py-2 rounded-lg border border-border bg-input-background"
                      style={{ fontSize: "0.875rem" }}
                    />
                  </div>
                    ...
                    <div className="p-3 rounded-lg bg-muted/50">
                      ...
                      >
                        ...
                        <span className="font-mono">
                          {(subscription as { tenantId?: string }).tenantId ||
                            "ten-001"}
                        </span>
                      </p>
                    </div>
            ...
            <div>
              <div className="mb-6">
                ...
                <p
                  ...
                  style={{ fontSize: "0.875rem" }}
                >
                  Academic year, terms, and scheduling.
                </p>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          {activeTab === "classes" && (
            <div className="bg-card border border-border rounded-xl p-6">
              <div className="space-y-2">
                {[
                  "Grade 10 â€” Sections: A, B",
                  "Grade 11 â€” Sections: A, B",
                  "Grade 12 â€” Sections: A",
                ].map((cls) => (
                  <div
                    key={cls}
                    className="flex items-center justify-between p-3 rounded-lg border border-border"
          ...
          {activeTab === "subjects" && (
            <div className="bg-card border border-border rounded-xl p-6">
              <div className="space-y-2">
                {[
                  "Mathematics â€” Algebra, Calculus",
                  "Science â€” Physics, Chemistry",
                  "English â€” Literature, Grammar",
                  "Social Studies â€” History, Geography",
                  "Computer Science â€” Programming, Data Structures",
                ].map((sub) => (
                  <div
                    key={sub}
                    className="flex items-center justify-between p-3 rounded-lg border border-border"
            ...
            <div>
              <div className="mb-6">
                ...
                <p
                  ...
                  style={{ fontSize: "0.875rem" }}
                >
                  Enable or disable modules for your organization. Core modules
                  cannot be disabled.
                </p>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p
                        ...
                        style={{ fontSize: "0.875rem" }}
                      >
                        Configure currency and library fine preferences.
                      </p>
                    </div>
                  </div>
                  <div className="grid gap-5">
                    ...
                    <div className="flex items-center justify-between py-3 border-b border-border">
                      <div>
                        ...
                        >
                          Symbol used for all monetary values across the
                          platform.
                        </p>
                      </div>
                      <select
                    ...
                    <div className="flex items-center justify-between py-3">
                      <div>
                        ...
                        <p
                          ...
                          style={{ fontSize: "0.8rem" }}
                        >
                          When enabled, overdue library books accrue fines
                          displayed in the library module.
                        </p>
                      </div>
                      <button
                        onClick={() =>
                          setLibraryFinesEnabled(!libraryFinesEnabled)
                        }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {modules.map((mod) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center ${moduleStates[mod.id] ? "bg-primary/10" : "bg-muted"}`}
                      >
                        <Globe
                          className={`w-5 h-5 ${moduleStates[mod.id] ? "text-primary" : "text-muted-foreground"}`}
                        />
                      </div>
                      <div>
                    ...
                    <button
                      onClick={() => !mod.core && toggleModule(mod.id)}
                      disabled={mod.core}
                      className={`${mod.core ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
                    >
                      {moduleStates[mod.id] ? (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card border border-border rounded-xl p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label style={{ fontSize: "0.875rem" }}>Min Length</label>
                      <select
                        className="w-full mt-1 px-3 py-2 rounded-lg border border-border bg-input-background"
                        style={{ fontSize: "0.875rem" }}
                      >
                ...
                <div className="lg:col-span-2 bg-card border border-border rounded-xl p-6">
                  ...
                  <p
                    ...
                    style={{ fontSize: "0.8125rem" }}
                  >
                    Auto-blocked IPs from brute force detection.
                  </p>
                  <div className="flex items-center justify-between p-3 rounded-lg border border-border">
                    <div>
                      ...
                      <span
                        ...
                        style={{ fontSize: "0.75rem" }}
                      >
                        Auto-blocked &middot; Feb 26, 2026
                      </span>
                    </div>
                    <button
                  ...
                  <button
                    className="mt-3 px-3 py-1.5 rounded-lg border border-border hover:bg-muted transition-colors"
                    style={{ fontSize: "0.8125rem" }}
                  >
                    + Add IP to blocklist
                  </button>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p
                    ...
                    style={{ fontSize: "0.875rem" }}
                  >
                    Third-party services, API keys, and sync settings.
                  </p>
                </div>
                <button
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {integ.status === "connected" && (
                          <button
                            className="px-2.5 py-1 rounded-lg border border-border hover:bg-muted"
                            style={{ fontSize: "0.75rem" }}
                          >
                        ...
                        <button
                          className={`px-2.5 py-1 rounded-lg ${integ.status === "connected" ? "border border-red-200 text-red-600 hover:bg-red-50" : "bg-primary text-primary-foreground hover:opacity-90"}`}
                          style={{ fontSize: "0.75rem" }}
                        >
                          {integ.status === "connected"
                            ? "Disconnect"
            ...
            <div>
              <div className="mb-6">
                ...
                <p
                  ...
                  style={{ fontSize: "0.875rem" }}
                >
                  Backup settings, data export, and management.
                </p>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div className="bg-card border border-border rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      <label style={{ fontSize: "0.875rem" }}>Frequency</label>
                      <select
                        className="w-full mt-1 px-3 py-2 rounded-lg border border-border bg-input-background"
                        style={{ fontSize: "0.875rem" }}
                      >
                    ...
                    <div>
                      <label style={{ fontSize: "0.875rem" }}>Retention</label>
                      <select
                        className="w-full mt-1 px-3 py-2 rounded-lg border border-border bg-input-background"
                        style={{ fontSize: "0.875rem" }}
                      >
                  ...
                  <button
                    ...
                    style={{ fontSize: "0.875rem" }}
                  >
                    <Database className="w-4 h-4" /> Manual Backup
                  </button>
                </div>
                ...
                <div className="bg-card border border-border rounded-xl p-6">
                  ...
                  <p
                    ...
                    style={{ fontSize: "0.8125rem" }}
                  >
                    Export for compliance, migration, or reporting.
                  </p>
                  <div className="space-y-2">
                    {[
                      { label: "All Users", format: "CSV" },
                      { label: "Academic Records", format: "Excel" },
                      { label: "Attendance Data", format: "CSV" },
                    ].map((exp) => (
                      <button
                        key={exp.label}
                ...
                <div className="space-y-2">
                  {recentBackups.map((backup) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span
                          className={`w-2 h-2 rounded-full ${backup.status === "completed" ? "bg-green-500" : "bg-red-500"}`}
                        />
                        <div>
                          <p style={{ fontWeight: 500, fontSize: "0.875rem" }}>
                            {backup.date}
                          ...
                          <p
                            ...
                            style={{ fontSize: "0.75rem" }}
                          >
                            {backup.size} &middot; {backup.type}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  {[
                    {
                      ...
                      desc: "Delete all logs older than 90 days",
                    },
                    {
                      title: "Reset Platform Data",
                      desc: "Clear all academic data (users preserved)",
                    },
                  ].map((item) => (
                    <div
                      key={item.title}
                      className="flex items-center justify-between p-3 rounded-lg border border-red-200"
                    >
                      <div>
                        <p style={{ fontWeight: 500, fontSize: "0.875rem" }}>
                          {item.title}
                    ...
                    {[
                      {
                        ...
                        description:
                          "When a new user is created or accepts an invite",
                        checked: true,
                      },
                      {
                      ...
                      {
                        label: "Storage threshold alerts",
                        description: "When storage usage exceeds 80%",
                        checked: true,
                      },
                      {
                        label: "SSL certificate expiry",
                        description: "30 days before SSL certificate expires",
                        checked: true,
                      },
                      {
                        label: "Integration errors",
                        description:
                          "When a connected integration encounters an error",
                        checked: true,
                      },
                      {
                        label: "Weekly usage report",
                        description: "Summary of platform activity and metrics",
                        checked: false,
                      },
                    ].map((item) => (
                      <label
    ...
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground" style={{ fontSize: "0.875rem" }}>
            Configure school profile and academic settings.
          </p>
        </div>
        <button
          onClick={handleSave}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${saved ? "bg-green-600 text-white" : "bg-primary text-primary-foreground hover:opacity-90"}`}
          style={{ fontSize: "0.875rem" }}
        >
      ...
      <div className="flex gap-1 mb-6 p-1 bg-muted rounded-lg w-fit overflow-x-auto">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`px-3 py-1.5 rounded-md whitespace-nowrap transition-colors ${activeTab === tab.id ? "bg-card shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
      ...
      {activeTab === "academic" && (
        <div className="bg-card border border-border rounded-xl p-6 space-y-5 max-w-2xl">
          ...
          <div>
            <label style={{ fontSize: "0.875rem" }}>Terms</label>
